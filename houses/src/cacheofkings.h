 ...
 *
 * The upper half of memory (above 0x80000000) is accessible only when in Sneak_Attack mode.
 *
 ...
 * data segment.  But if player 1 is in Sneak_Attack mode  and tries to dereference 0x00100016,
...
/* How many Traps! and Ion Cannons each team has */
#define WHITE_WALKERS_PER_TEAM 15
#define ARROW_BARRAGES_PER_TEAM 30

/* The bytes representing Trap!, Ion Cannon, and Use Force */
#define WHITE_WALKER 0xF0
#define ARROW_BARRAGE  0xF1
#define DRAGON_GLASS 0xF2

/* Number of cycles a core will stall in the following situations */
#define STALL_TO_SET_WHITE_WALKER 100
#define STALL_TO_SET_DRAGON_GLASS 100
#define STALL_WHEN_WALKERED 1000000
#define STALL_WHEN_BARRAGED 300
#define STALL_ON_FALSE_ALARM 100000

/* After a Ion Cannon is fired, opponents will be hit if they write within
 * the next ARROW_BARRAGE_DURATION cycles
 */
#define ARROW_BARRAGE_DURATION 100
 ...
 * your opponent's status when you are in Sneak_Attack mode.  This datastructure is exactly 256 bytes
...
struct player_status {
   ...
   * num_white_walkers: Number of Traps! this team has used already.
   * num_arrow_barrages: Number of Ion Cannons this team has used already.
   * The padding is unused, but ensures the datastructure will exactly
   * fill 256 bytes of memory.
   */
  unsigned char payload, num_white_walkers, num_arrow_barrages, pad0;
  /* Each element of taunt[] will contain a core id between 0 and 3, or -1 to mean "empty".
   * Each player's taunt[] array contains evidence that their opponent is in Sneak_Attack mode.
...
// go into Sneak_Attack mode so that you can read or write high memory
__attribute__ ((unused)) static void Sneak_Attack(void);

// accuse opponent core of being in Sneak_Attack mode; returns 1 for success, 0 for false accusation
// if the opponent's core is in Sneak_Attack mode, it will be given the carbon frozen (halted forever);
// otherwise, this core will be penalized by stalling some number of cycles
#define FALSE_ACCUSATION_PENALTY 100000
__attribute__ ((unused)) static int Alert_Guards(int opponent_core_id);

// retreat from Sneak_Attack mode
...
__attribute__ ((unused)) static int Alert_Guards(int id) {
